...
def main() -> None:
    ...
    auto_inputs = read_input_file(this_console)
    ...
    num_auto_entries = len(auto_inputs)
    # this_console.print(f'number of entries is {num_auto_entries}')
    auto_bot_names = []
    auto_user_colors = []
    auto_recs_to_beat = []
    auto_target_record = []
    for line in auto_inputs:
        auto_bot_names.append(line)
        auto_user_colors.append(Player_Color.WHITE if auto_inputs[line][0] == 'w' else Player_Color.BLACK)
        auto_recs_to_beat.append(auto_inputs[line][1])
        auto_target_record.append(auto_inputs[line][2])

    for entry_key in range(num_auto_entries):
        user_color = auto_user_colors[entry_key]
        displayed_bot_name = auto_bot_names[entry_key]
        html_bot_name = mapping_inputs[displayed_bot_name][0]
        html_section_name = mapping_inputs[displayed_bot_name][1]
        record_to_beat = auto_recs_to_beat[entry_key]
        target_record = auto_target_record[entry_key]
        if entry_key > 0:
            ...
            while between_games_loop:
                ...
                while within_game_loop:
                    if full_move_number == 1:
                        print_game_number(num_games_played + 1, displayed_bot_name, entry_key + 1, num_auto_entries)
